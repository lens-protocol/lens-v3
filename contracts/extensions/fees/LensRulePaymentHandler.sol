...
import {CONTRACT__LENS_PAYMENT_HELPER, NATIVE_TOKEN} from "contracts/core/types/Constants.sol";
...
contract LensRulePaymentHandler is LensPaymentHandler {
    ...
    ILensNativeHelper private immutable LENS_PAYMENT_HELPER;
    ...
    function _sendToken(address token, address payer, address recipient, uint256 amount) internal virtual override {
        if (token == NATIVE_TOKEN) {
            LENS_PAYMENT_HELPER.transferNativeTo(recipient, amount);
        } else {
